...
Runner.prototype.runFile = function (f, dir, cb) {
  var self = this
...
Runner.prototype.filesWithOnly = function (files, cb) {
  files = files.slice(0)
  var only = []
  function next () {
...
Runner.prototype.runFiles = function (files, dir, cb) {
  ...
  self.filesWithOnly(files, function(err, filesOnly){
    if (filesOnly.length > 0){
      files = filesOnly
    }
    chain(files.map(function(f) {
      return function (cb) {
        self.runFile(f, dir, cb)
      }
    }), cb)
  })
